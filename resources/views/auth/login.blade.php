<x-guest-layout>
    <div class="text-left mb-8 border-b border-zinc-100 dark:border-zinc-800 pb-6">
        ...
        <p class="mt-2 text-[10px] font-bold text-zinc-400 uppercase tracking-[0.3em] leading-relaxed">
            {{ __('Masukkan kredensial Anda untuk mengakses Pusat Kendali DSM Core.') }}
        </p>
    </div>
    ...
    <form method="POST" action="{{ route('login') }}" class="space-y-6">
        ...
        <div class="space-y-2 text-left">
            ...
            <input id="email" name="email" type="email" 
                ...
                value="{{ old('email') }}" required autofocus autocomplete="username" placeholder="personel@dsm.com" />
            @if($errors->has('email'))
        ...
        <div class="space-y-2 text-left">
            <div class="flex justify-between items-center px-2">
                <label for="password" class="text-[10px] font-black text-zinc-400 dark:text-zinc-500 uppercase tracking-widest">
                    {{ __('Kunci Akses') }}
                </label>
                @if (Route::has('password.request'))
                    <a href="{{ route('password.request') }}" class="text-[9px] font-black text-zinc-400 hover:text-zinc-800 dark:hover:text-white uppercase tracking-widest transition-colors">
                        {{ __('Lupa Kunci?') }}
                    </a>
                @endif
            </div>
        ...
        <div class="flex items-center ml-2">
            <input id="remember_me" type="checkbox" name="remember" class="rounded-md border-zinc-300 dark:border-zinc-700 text-zinc-900 shadow-sm focus:ring-zinc-500 dark:bg-zinc-950">
            <label for="remember_me" class="ml-2 text-[10px] font-black text-zinc-400 dark:text-zinc-500 uppercase tracking-widest cursor-pointer">
        ...
        <div class="flex flex-col gap-4 pt-4">
            ...
            @if (Route::has('register'))
                <div class="text-center">
                    <a class="text-[10px] font-black text-zinc-400 dark:text-zinc-500 uppercase tracking-widest hover:text-zinc-800 dark:hover:text-white transition-colors" href="{{ route('register') }}">
                        {{ __('Belum Terdaftar? Inisiasi Akun Baru') }}
                    </a>
                </div>
            @endif
        </div>
    ...
    @if (session('status'))
        <div class="mt-8 p-4 rounded-2xl bg-emerald-50 dark:bg-emerald-500/5 border border-emerald-100 dark:border-emerald-500/10 flex items-center gap-3">
            <div class="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse"></div>
            <p class="text-[9px] font-black text-emerald-700 dark:text-emerald-400 uppercase tracking-[0.2em] italic">{{ session('status') }}</p>
        </div>
    @endif
</x-guest-layout>